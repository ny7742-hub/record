...
- 재고이관 분석 (`🔄 재고이관` 탭): PlayMD 창고재고현황 + 기간별BestWorst판매현황 업로드 →
  ...
  선택 업로드 `매장별 판매현황`(매장 × 상품코드 × 판매수량)이 있으면 매장별 판매속도를 실측으로 계산.
  없으면 일별매출의 매장 매출 비중으로 안분 추정 — 자비스에는 SKU별 매장 판매 이력이 없다
  (`dailySalesDB`/`deptDB`는 매출액만, 판매일보 장바구니는 PlayMD 업로드용이라 미저장).
